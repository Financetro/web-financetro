...
function CMSContent({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Multiple ways to detect App Mode:
    // 1. Query Parameters
    const mode = searchParams.get("mode");
    const appParam = searchParams.get("app");

    // 2. User Agent check for Android WebView
    const ua = window.navigator.userAgent.toLowerCase();
    const isAndroidWebView = ua.includes("android") && (ua.includes("wv") || ua.includes("version/"));

    if (mode === "app" || appParam === "true" || isAndroidWebView) {
      setIsApp(true);
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#06080C] text-white selection:bg-primary/30 font-sans relative">
      ...
      <main className={`flex-1 relative z-10 container mx-auto px-4 ${isApp ? 'py-10' : 'py-20'} max-w-4xl transition-all duration-300`}>
